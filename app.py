...
def generate_invoice_pdf(person_name, transactions_df, invoice_type_display, start_date=None, end_date=None):
    ...
    try:
        ...
        pdf.set_text_color(0, 123, 255) # Blue for company name

        # Company Info
        company_name = "Payment Tracker Systems Inc."
        company_address = "123 Financial Lane, Business City, BC 12345"
        company_email = "info@paymenttracker.com"
        company_phone = "+1 (555) 123-4567"

        pdf.cell(0, 10, company_name, ln=True, align='C')
        pdf.set_font("Arial", size=10)
        pdf.set_text_color(85, 85, 85) # Grey for address
        pdf.cell(0, 5, company_address, ln=True, align='C')
        pdf.cell(0, 5, company_email, ln=True, align='C')
        ...
        pdf.cell(0, 7, f"Invoice #: INV-{invoice_id}", ln=True, align='R')
        pdf.cell(0, 7, f"Date: {invoice_date}", ln=True, align='R')
        pdf.ln(15)
        ...
        if not transactions_df.empty:
            ...
            for idx, row in transactions_df_sorted.iterrows():
                ...
                pdf.cell(col_widths[0], 10, str(row['date']), 1, 0, 'L')
                pdf.cell(col_widths[1], 10, str(row['description'] if row['description'] else '-'), 1, 0, 'L')
                pdf.cell(col_widths[2], 10, str(row['type_display']), 1, 0, 'L')
                pdf.cell(col_widths[3], 10, f"Rs. {row['amount']:,.2f}", 1, 1, 'R')
        else:
            pdf.cell(sum(col_widths), 20, "No transactions found for the selected criteria.", 1, 1, 'C')
        ...
        pdf.set_text_color(127, 140, 141) # Light grey
        pdf.cell(0, 5, "Thank you for your business!", ln=True, align='C')
        pdf.cell(0, 5, f"© {datetime.now().year} {company_name}. All Rights Reserved.", ln=True, align='C')
        ...
        if generate_invoice_button:
            ...
            if st.session_state['invoice_person_name'] == "Select a person...":
                ...
                try:
                    ...
                    all_transactions_df = clean_payments_data(all_transactions_df) # Ensure data is clean

                    # Filter by person
                    person_transactions_df = all_transactions_df[all_transactions_df['person'] == st.session_state['invoice_person_name']].copy()
                    
                    # Prepare for display (adds 'type_display', 'amount_display', etc.)
